...
def test_order_journal_record_update_and_query():
    temp_root = Path(".tmp") / "pytest" / "order_journal"
    temp_root.mkdir(parents=True, exist_ok=True)
    temp_dir = temp_root / f"order_journal_{uuid4().hex}"
    temp_dir.mkdir(parents=True, exist_ok=True)
    db_file = temp_dir / "orders.sqlite"
